...
class RecentExerciseResourceList(Resource):
    ...
    def get(self):
        ...
        query_result = RecentRiffExercise.query.filter(RecentRiffExercise.created_by == current_user.id).order_by(desc(RecentRiffExercise.modified_at)).all()
    ...
    def post(self):
        ...
        query_result = RecentRiffExercise.query.filter(RecentRiffExercise.created_by == current_user.id).filter(RecentRiffExercise.riff_exercise_id == riff_exercise_id).all()
        if len(query_result) == 0:
            logger.info("adding recent exercise", riff_exercise_id=riff_exercise_id, user_id=current_user.id)
            riff = RecentRiffExercise(id=str(uuid.uuid4()), riff_exercise_id=riff_exercise_id, created_by=current_user.id)
            save(riff)

            # clean to ensure max recent exerciss count is 5
            logger.info("cleaning recent exercises", user_id=current_user.id)
            query_result = RecentRiffExercise.query.filter(RecentRiffExercise.created_by == current_user.id).order_by(desc(RecentRiffExercise.modified_at)).offset(5)
